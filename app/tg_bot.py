...
def build_dispatcher(ctx: Context) -> Dispatcher:
    ...
    async def _cmd_help(message: Message) -> None:
        ...
        txt = (
            ...
            "/link <max_id> — привязать <b>текущую</b> тему к MAX‑чату\n"
            "/unlink — отвязать текущую тему (выполнять внутри темы)\n"
            "/relink <max_id> — пересоздать тему и привязать (если тема пропала)\n\n"
            ...
            "/add_forward <tg_channel_id> <max_chat_id> [name] — добавить форвард\n"
            "/remove_forward <tg_channel_id> — удалить форвард\n"
            ...
            "/presence [<user_id>] — статус контактов (живая лента в теме «MAX presence»)\n\n"
            "<b>Как узнать ID:</b>\n"
